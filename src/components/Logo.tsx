...
export default function Logo({ className = '', showSubtitle = true, variant = 'dark', settings, size }: LogoProps) {
  ...
  const renderIcon = () => {
    if (customUrl) {
      return (
        <motion.div 
          className="relative w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg sm:rounded-xl overflow-hidden shadow-2xs shrink-0"
    ...
    switch (iconType) {
      ...
      case 'symbol-shield':
        return (
          <motion.div 
            className={`w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg sm:rounded-xl ${theme.lightBg} ${theme.primaryText} border ${theme.primaryBorder}/20 shadow-inner shrink-0`}
            whileHover={{ scale: 1.08, rotate: 5 }}
            transition={{ type: "spring", stiffness: 300 }}
          >
            <Icons.ShieldAlert className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 animate-pulse" />
          </motion.div>
        );
      ...
      case 'symbol-sparkles':
        return (
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 300 }}
          >
            <Icons.Sparkles className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 animate-bounce" />
          </motion.div>
        );
      ...
      case 'symbol-crown':
        return (
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 300 }}
          >
            <Icons.Crown className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-amber-500" />
          </motion.div>
        );
      ...
      case 'symbol-bolt':
        return (
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 300 }}
          >
            <Icons.Zap className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-amber-400 fill-amber-400 animate-pulse" />
          </motion.div>
        );
      ...
      default:
        return (
          <motion.div 
            className="relative w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg sm:rounded-xl p-0.5 bg-transparent select-none cursor-pointer shrink-0"
            whileHover={{ 
              scale: 1.08,
          ...
          >
            <svg
              ...
              className="w-6 h-6 xs:w-7 xs:h-7 sm:w-9 sm:h-9 md:w-11 md:h-11 overflow-visible max-w-full"
              id="dizo-logo-svg"
            >
              <defs>
  ...
  const wordmarkSizeClass = size === 'sm'
  ...
  return (
    <div className={`flex flex-col items-start min-w-0 max-w-full overflow-hidden shrink ${className}`} id="dizo-logo-wrapper">
      <div className="flex items-center gap-1.5 xs:gap-2 sm:gap-3 min-w-0 max-w-full">
        {renderIcon()}

        {/* Wordmark */}
        <div className="flex flex-col min-w-0 truncate">
          <div className={`${wordmarkSizeClass} font-black tracking-tight xs:tracking-normal sm:tracking-wider font-sans leading-none flex items-center whitespace-nowrap`}>
            <span className={`${word1ColorClass} font-black uppercase`}>{textFirst}</span>
            <span className={`${word2ColorClass} font-black uppercase ml-0.5 xs:ml-1 sm:ml-1.5 filter drop-shadow-sm`}>{textSecond}</span>
          </div>
          {showSubtitle && (
            <span className={`text-[7px] xs:text-[8px] sm:text-[9px] uppercase tracking-[0.14em] xs:tracking-[0.18em] sm:tracking-[0.25em] font-bold leading-none mt-1 sm:mt-1.5 whitespace-nowrap truncate max-w-full ${isDark ? 'text-slate-500' : 'text-indigo-200'}`}>
              {subtitleText}
            </span>
          )}
        </div>
