...
export const LeadsCrmPipeline: React.FC<LeadsCrmPipelineProps> = ({
  ...
  }, [inquiries, showArchived, searchTerm, stageFilter, priorityFilter, staffFilter, serviceFilter, dateRangeFilter]);
  ...
  const handleExportCSV = (targetLeads?: Inquiry[]) => {
    const listToExport = targetLeads || (selectedInquiryIds.length > 0
      ? inquiries.filter(i => selectedInquiryIds.includes(i.id))
      : filteredInquiries);
        ...
        <div className="overflow-x-auto pb-6">
          <div className="flex gap-4 min-w-[1400px]">
            {STAGES.map(stage => {
              ...
              const stageValue = stageLeads.reduce((acc, curr) => acc + (curr.totalDiscounted || curr.totalOriginal || 0), 0);
              ...
              return (
                <div
                  ...
                  className={`w-72 shrink-0 rounded-2xl border ${stage.borderColor} ${stage.bgColor} p-3 flex flex-col max-h-[800px] min-h-[500px] shadow-2xs`}
                >
                  {/* Column Header */}
                  <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-200/60">
                    <div className="flex items-center gap-2">
                      <div className={`p-1.5 rounded-lg bg-white shadow-2xs ${stage.textColor}`}>
                    ...
                    <span className="px-2 py-0.5 bg-white text-slate-800 text-[11px] font-black rounded-full shadow-2xs border border-slate-200">
                      {stageLeads.length}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                    {stageLeads.length === 0 ? (
                      <div className="h-32 border-2 border-dashed border-slate-200/80 rounded-xl flex items-center justify-center text-[11px] text-slate-400 font-medium italic text-center p-4">
                        Drag leads here or select stage
                      </div>
                    ) : (
                      stageLeads.map(lead => {
                        const leadVal = lead.totalDiscounted || lead.totalOriginal || 0;
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                              <div>
                                ...
                                <span className="font-black text-slate-900">₹{leadVal.toLocaleString('en-IN')}</span>
                              </div>
